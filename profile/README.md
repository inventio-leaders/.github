...
* [Импорт и обработка Excel](#импорт-из-excel-входные-форматы)
* [Дедупликация и загрузка](#дедупликация-и-загрузка-в-бд)
* [Ошибки и валидация](#ошибки-и-сообщения-валидации)
* [Типовые сценарии](#типовые-сценарии-использования)
* [Прогноз и аномалии](#запуск-анализа)
...
```
df['Отклонение_%'] = (df['Разница'].abs() / ((df['ХВС_ИТП'] + df['ГВС_ОДПУ']) / 2)) * 100
```

 `df['Отклонение_%']` больше 10% становится аномалией.
...
## Импорт из Excel: входные форматы
